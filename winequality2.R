...
for (j in 1:(length(cleanRedwine)-1)){
  variable <- colnames(cleanRedwine[j])
  for (i in cleanRedwine[j]){
...
for (i in 1:nrow(outlier)){
  if (outlier[i,2]/nrow(cleanRedwine) * 100 >= 5){
    print(paste(outlier[i,1], 
                '=', 
                round(outlier[i,2]/nrow(cleanRedwine) * 100, digits = 2),
                '%'))
  }
}
...
for (i in 4:5){
  for (j in 1:nrow(cleanRedwine)){
    if (cleanRedwine[[j, i]] > as.numeric(quantile(cleanRedwine[[i]], 0.75) + 
                                  1.5 * IQR(cleanRedwine[[i]]))){
      if (i == 4){
        cleanRedwine[[j, i]] <- round(mean(cleanRedwine[[i]]), digits = 2)
      } else{
        cleanRedwine[[j, i]] <- round(mean(cleanRedwine[[i]]), digits = 3)
      }
    }
  }
}
...
#Tuning parameter 'sigma' was held constant at a value of 0.07510276
#RMSE was used to select the optimal model using the smallest value.
#The final values used for the model were sigma = 0.07510276 and C = 1.
